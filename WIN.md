...
The output is generated in directory **`target\sl\`** and looks as follows:
...
Command [**`generate_parser`**](generate_parser.bat) with no arguments produces the lexer and the parser for the [**`SimpleLanguage`**](https://github.com/graalvm/simplelanguage) example.
...
Command [**`generate_parser -verbose test`**](generate_parser.bat) generates a test class **`SimpleLanguageMainTest.java`** and compiles/executes it together with the output files from **`target\parser\`**.

<pre style="font-size:80%;">
<b>&gt; generate_parser -verbose test</b>
Generate ANTLR parser files into directory W:\target\parser
Generate test class SimpleLanguageMainTest.java into directory W:\target\parser
Compile Java source files to directory W:\target\classes
...
> **:mag_right:** Command [**`generate_parser test`**](generate_parser.bat) depends on file **`simplelanguage-19.1.1-SNAPSHOT.jar`** which is generated by command [**`build.bat`**](build.bat).
> <pre style="font-size:80%;">
> <b>&gt; generate_parser test</b>
> Error: File not found (language\target\simplelanguage-19.1.1-SNAPSHOT.jar)
> </pre>
> Running command [**`build.bat`**](build.bat) will generate the missing file in directory **`language\target\`**.
